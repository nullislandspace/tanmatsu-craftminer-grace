...
| | **Open, and the user asked about it:** **ore does not come in veins.** `cm_rand3` hashes the individual cell, so every ore block is an independent coin flip -- iron below y = 28 at 0.65% of stone cells, coal below y = 40 at 1.2%, both scattered one block at a time. Minecraft generates veins (coal 4-16, iron 1-10). The comment in `worldgen.c` said "a coal seam", which was simply wrong, and has been corrected. Making them clump is a pass of its own -- origins on a coarse grid, a short random walk from each -- and it changes what new chunks look like, so it waits on the user's word. | | |
