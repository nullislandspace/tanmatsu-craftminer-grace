// =====================================================================
//  SynthMiner  --  seeded noise (see rng.h)
// =====================================================================
...
float sm_noise2(float x, float z, float scale, uint32_t seed) {
    ...
    float const n00 = sm_rand2(ix, iz, seed);
    float const n10 = sm_rand2(ix + 1, iz, seed);
    float const n01 = sm_rand2(ix, iz + 1, seed);
    float const n11 = sm_rand2(ix + 1, iz + 1, seed);
...
float sm_noise3(float x, float y, float z, float scale, uint32_t seed) {
    ...
    float const c000 = sm_rand3(ix, iy, iz, seed);
    float const c100 = sm_rand3(ix + 1, iy, iz, seed);
    float const c010 = sm_rand3(ix, iy + 1, iz, seed);
    float const c110 = sm_rand3(ix + 1, iy + 1, iz, seed);
    float const c001 = sm_rand3(ix, iy, iz + 1, seed);
    float const c101 = sm_rand3(ix + 1, iy, iz + 1, seed);
    float const c011 = sm_rand3(ix, iy + 1, iz + 1, seed);
    float const c111 = sm_rand3(ix + 1, iy + 1, iz + 1, seed);
...
float sm_fbm2(float x, float z, float scale, int octaves, uint32_t seed) {
    ...
    for (int i = 0; i < octaves; i++) {
        ...
        sum += amp * sm_noise2(x, z, s, seed + (uint32_t)i * 0x9E3779B9u);
        norm += amp;
        amp *= 0.5f;
        s *= 0.5f;
    }
