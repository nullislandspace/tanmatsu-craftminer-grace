#pragma once
// =====================================================================
//  SynthMiner  --  seeded hashing, noise and random streams
// ---------------------------------------------------------------------
...
//  determinism rule 2 (claudeplans/synthminer.md, Part T), and it is
...
// an int32 can hold.
static inline uint32_t sm_hash2(int32_t x, int32_t z, uint32_t seed) {
    uint64_t h = (uint64_t)(uint32_t)x;
    h = sm_mix64(h ^ ((uint64_t)(uint32_t)z << 32));
    return (uint32_t)(sm_mix64(h ^ seed) >> 32);
}

static inline uint32_t sm_hash3(int32_t x, int32_t y, int32_t z, uint32_t seed) {
    uint64_t h = (uint64_t)(uint32_t)x | ((uint64_t)(uint32_t)z << 32);
    h = sm_mix64(h ^ ((uint64_t)(uint32_t)y * 0xD6E8FEB86659FD93ULL));
    return (uint32_t)(sm_mix64(h ^ seed) >> 32);
}

// The same, as a float in [0, 1).
static inline float sm_rand2(int32_t x, int32_t z, uint32_t seed) {
    return (float)(sm_hash2(x, z, seed) >> 8) * (1.0f / 16777216.0f);
}
static inline float sm_rand3(int32_t x, int32_t y, int32_t z, uint32_t seed) {
    return (float)(sm_hash3(x, y, z, seed) >> 8) * (1.0f / 16777216.0f);
}
...
static inline void sm_rng_seed(sm_rng_t* r, uint64_t seed) {
    r->s = seed ? seed : 0x9E3779B97F4A7C15ULL;  // never zero: xorshift would stick
}

static inline uint32_t sm_rng_u32(sm_rng_t* r) {
    uint64_t x = r->s;
    x ^= x >> 12;
    x ^= x << 25;
    x ^= x >> 27;
...
static inline float sm_rng_f(sm_rng_t* r) {
    return (float)(sm_rng_u32(r) >> 8) * (1.0f / 16777216.0f);
}

// 0 .. n-1, unbiased enough for gameplay (n is always small here).
static inline uint32_t sm_rng_below(sm_rng_t* r, uint32_t n) {
    return n ? sm_rng_u32(r) % n : 0u;
}
...
float sm_noise2(float x, float z, float scale, uint32_t seed);

// The 3D version, for density fields (caves, the Far Lands).
float sm_noise3(float x, float y, float z, float scale, uint32_t seed);

// Several octaves of sm_noise2, each half the amplitude and twice the
// frequency of the last. Result in [0, 1).
float sm_fbm2(float x, float z, float scale, int octaves, uint32_t seed);
